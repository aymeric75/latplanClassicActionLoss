...
Poor python coders cannot enjoy the cleanness of CLOS :before, :after, :around methods."""
    ...
    def bar_update(self, batch, logs):
        ...
        if (batch % 10) == 9:
            self.bar.update(batch+1, status = "[v] "+"  ".join(["{} {:8.3g}".format(k,v) for k,v in sorted(vlogs.items())]) + "\n")
 ...
 This method may be overloaded by the subclass into a specific training method, e.g. GAN training."""
        ...
        def generate_logs(data, data_to):
            ...
            images_np_array = np.array(images_array)

            actions_array = []

            for d in data:

                row = np.tile(d[1], 2) # (1, 48)
                matrix = np.tile(row, (48, 1))
              
                actions_array.append(matrix)

            actions_np_arr = np.array(actions_array)

            
            actions_np_arr = np.expand_dims(actions_np_arr, axis=-1)
            actions_np_arr = np.expand_dims(actions_np_arr, axis=1)

            theinput = np.concatenate((images_np_array, actions_np_arr), axis=1)

            # les actions (900, 1, 48, 48, 1)
            ...
            # attend (3, 48, 48, 1)
            evals = self.nets[0].evaluate(theinput,
                                    images_np_array,
                                    batch_size=batch_size,
                                    verbose=0)
            #print(evals)
            # [312953506001.35114, 5.0, 0.05444444444444444]
        ...
        try:
            ...
            for epoch in range(start_epoch,start_epoch+epoch):
                ...
                # print(np.array(train_data_cache).shape) # (900, 1, 2)
                ...
                for train_subdata_cache,train_subdata_to_cache in zip(train_data_cache,train_data_to_cache):
                    ...
                    for item in train_subdata_cache:
                        #print(item[1].shape) # (24,)
                        row = np.tile(item[1], 2)
                        matrix = np.tile(row, (48, 1))
                    
                        action_input_data.append(matrix)
                    ...
                    # print("x_data.shape")
                    # print(x_data.shape) # (1, 2, 48, 48, 1)
              
                    action_input_data = np.expand_dims(action_input_data, axis=-1)
                    action_input_data = np.expand_dims(action_input_data, axis=1)
                  

                    resu = np.concatenate((x_data, action_input_data), axis=1)
